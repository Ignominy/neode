...
function Create(neode, model, properties, extraEagerNames, customerId) {
  return (0, _GenerateDefaultValues["default"])(neode, model, properties).then(function (properties) {
    ...
    (0, _WriteUtils.addNodeToStatement)(neode, builder, alias, model, properties, [alias], "create", [], customerId); // Output
    ...
    return builder["return"](output).execute(_Builder.mode.WRITE).then(function (res) {
      if (!res) throw new Error("Could not create node for model ".concat(model.name()));
