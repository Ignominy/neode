...
exports["default"] = DeleteByProperties;
...
function addCascadeDeleteNode(neode, builder, from_alias, relationship, aliases, curr_depth, to_depth, customerId) {
  ...
  builder.optionalMatch(from_alias).relationship(relationship.relationship(), relationship.direction(), rel_alias).to(node_alias, relationship.target(), undefined, customerId); // Check for cascade deletions

  target.relationships().forEach(function (relationship) {
    switch (relationship.cascade()) {
      case "delete":
        var next_depth = curr_depth + 1;
...
function addDetachNode(neode, builder, from_alias, relationship, aliases, customerId) {
  ...
  builder.optionalMatch(from_alias).relationship(relationship.relationship(), relationship.direction(), rel_alias).to(node_alias, relationship.target(), undefined, customerId)["delete"](rel_alias);
  builder.withDistinct(aliases);
}
/**
...
function DeleteByProperties(neode, model, properties, customerId) {
  ...
  (0, _ReadUtils.addReadNodeToStatement)(neode, builder, alias, model, properties, undefined, aliases, customerId); // Cascade delete to relationships
