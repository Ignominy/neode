...
function addDetachNode(neode, builder, from_alias, relationship, aliases) {
  ...
  builder.optionalMatch(from_alias).relationship(relationship.relationship(), relationship.direction(), rel_alias).to(node_alias, relationship.target())["delete"](rel_alias); // builder.withDistinct( aliases );
}
/**
...
function DeleteNode(neode, identity, model, customerId) {
  ...
  var builder = new _Builder["default"](neode).match(alias, model, undefined, customerId).whereId(alias, identity); // Cascade delete to relationships
