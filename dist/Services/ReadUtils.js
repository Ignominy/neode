...
function checkProperties(model, properties) {
  var inline = {}; // Calculate Set Properties
...
function addReadNodeToStatement(neode, builder, alias, model, properties, order) {
  ...
  var inline = checkProperties(model, properties); // Add alias
  ...
  builder.match(alias, model, inline, customerId); // Add order statement if exists
      ...
      switch (relationship.type()) {
        ...
        case "relationships":
          ...
          value.forEach(function (value, idx) {
            ...
            aliases.push(targetAlias); // Carry alias through
...
function addReadRelationshipToStatement(neode, builder, alias, rel_alias, target_alias, relationship, value, aliases, customerId) {
  ...
  delete value[node_alias]; // If Node is passed, attempt to match a relationship to that specific node
