...
export default class Node extends Entity {
   ...
   */
  delete(to_depth) {
    return DeleteNode(this._neode, this._identity, this._model, to_depth).then(() => {
      this._deleted = true
  ...
  relateTo(node, type, properties = {}, force_create = false) {
    ...
    return RelateTo(this._neode, this, node, relationship, properties, force_create).then(rel => {
