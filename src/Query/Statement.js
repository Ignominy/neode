...
export default class Statement {
  ...
  toString(includePrefix = true) {
    ...
    if (this._pattern.length) {
      ...
      output.push(this._pattern.map(statement => statement.toString()).join(""))
    }

    if (this._where.length) {
      output.push(this._where.map(statement => statement.toString()).join(""))
    }
    ...
    if (this._on_create_set.length) {
      ...
      output.push(this._on_create_set.map(output => output.toString()).join(", "))
    }

    if (this._on_match_set.length) {
      output.push("ON MATCH SET")

      output.push(this._on_match_set.map(output => output.toString()).join(", "))
    }

    if (this._set.length) {
      output.push("SET")

      output.push(this._set.map(output => output.toString()).join(", "))
    }

    if (this._delete.length) {
      output.push("DELETE")

      output.push(this._delete.map(output => output.toString()))
    }

    if (this._detach_delete.length) {
      output.push("DETACH DELETE")

      output.push(this._detach_delete.map(output => output.toString()))
    }

    if (this._return.length) {
      output.push("RETURN")

      output.push(this._return.map(output => output.toString()))
    }

    if (this._order.length) {
      output.push("ORDER BY")

      output.push(this._order.map(output => output.toString()))
    }
